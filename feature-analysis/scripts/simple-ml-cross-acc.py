...
def load_source(source: str) -> tuple[float, pd.DataFrame]:
    feature_csv = OUT / f"{source}-summary-au-data.csv"
    results_csv = OUT / f"{source}-simple-ml-data-run-summary.csv"
...
def main() -> None:
    ...
    for (sx, sy) in itertools.combinations(SOURCES, 2):
        cli.chapter(f"[{sx} x {sy}]")

        random_x, ml_x = load_source(sx)
        random_y, ml_y = load_source(sy)
        ...
        cli.print(ml_sorted)
        ml_sorted.to_csv(OUT / f"{sx}-vs-{sy}-simple-ml-test-acc.csv")
        ...
        fig.savefig(PLOT_DIR / f"{sx}-vs-{sy}-simple-ml-test-acc.png")
        plt.close()
